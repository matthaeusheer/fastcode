...
Test(pso_basic_integration, rosenbrock) {
  ...
  double* mins = filled_array(dim, -30);
  double* maxs = filled_array(dim, 30);
...
/*
Test(squirrel_integration, egghol2d) {
  size_t population = POPULATION;
  size_t dim = 2;
  size_t max_iter = 250;
  double* mins = filled_array(dim, -512);
  double* maxs = filled_array(dim, 512);
  double* solution = squirrel(egghol2d, population, dim, max_iter, mins, maxs);
...
/*
Test(squirrel_integration, rastigrin) {
  size_t population = POPULATION;
  size_t dim = DIM;
  size_t max_iter = 250;
  double* mins = filled_array(dim, -20);
  double* maxs = filled_array(dim, 10);
  double* solution = squirrel(rastigrin, population, dim, max_iter, mins, maxs);
...
Test(pso_basic_integration, griewank) {
  ...
  double* mins = filled_array(dim, -5);
  double* maxs = filled_array(dim, 5);
  double* solution = pso_basic(griewank, population, dim, max_iter, mins, maxs);
