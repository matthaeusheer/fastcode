...
double *pen_emperor_penguin(obj_func_t obj_func,
  ...
  double* r_matrix = (double*)malloc(dim*dim*sizeof(double));
  ...
  for (size_t iter = 0; iter < max_iterations; iter++) {
    ...
    double* updated_positions = (double*)malloc(colony_size*colony_size*dim*sizeof(double));
    ...
    for (size_t pengu_idx = 0; pengu_idx < colony_size; pengu_idx++) {
      // double* mean_pos = (double*)malloc(dim*sizeof(double));
      double mean_pos[dim];
